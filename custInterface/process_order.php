<?php 
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_POST['order_info'])) {
			...
			$pricetotal	= htmlentities($_POST['price'], ENT_QUOTES, 'UTF-8');
			
			if($name != "" && $addr != "" && $email != "" && $phone != "" && $food != "" && $pricetotal != "") {
				
				$insert = $db->query("INSERT INTO basket(customer_name, contact_number, address, email, total, status, date_made) VALUES('".$name."', '".$phone."', '".$addr."', '".$email."', '".$pricetotal."', 'pending', NOW())");
